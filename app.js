var lppModule = (function() {
  ...
  function _toggleScrolledClassOnScroll() {
    ...
    document
      ...
      .removeEventListener(_eventName, _toggleMenu, false);
    document
      .querySelector(".hamburger")
      .addEventListener(_eventName, _toggleMenu, false);
  ...
  function _addClickOrTouchendEventListeners() {
    ...
    document
      ...
      .addEventListener(_eventName, _toggleMenu, false);
    document
      .querySelector(".toggle-dropdown-btn")
      .addEventListener(_eventName, _toggleLanguagesDropDown, false);

    const closeAlertButtons = document.querySelectorAll(".close-alert-button");
    for (let i = 0; i < closeAlertButtons.length; i++) {
      closeAlertButtons[i].addEventListener(
        _eventName,
        _addCloseAlertEvent,
        false
      );
    ...
    document
      ...
      .addEventListener(_eventName, _closeEngagementModal, false);
  ...
  function _addFormSubmitListeners() {
    ...
    bottomContactForm.addEventListener("submit", event => {
      ...
      const email = bottomContactForm.querySelector("input[name='email']")
        .value;
  ...
  function _setClickOrTouchendEventName() {
    if (
      ...
      _platformType === _PlatformTypeEnum.MOBILE
    ) {
      _eventName = "touchend";
    } else if (
      _platformType === _PlatformTypeEnum.SAFARI_DESKTOP ||
      _platformType === _PlatformTypeEnum.DESKTOP
    ) {
      _eventName = "click";
    } else {
      _eventName = "click";
      ...
      setTimeout(() => {
        ...
        Array.from(asyncImages).map(asyncImage => {
          ...
          if (asyncImage.nodeName === "PICTURE") {
            ...
            const style =
              asyncImage.currentStyle ||
              window.getComputedStyle(asyncImage, false);
          ...
          img.addEventListener("load", event => {
            ...
            if (asyncImage.nodeName === "PICTURE") {
              ...
              const pictureSrc = asyncImage.querySelector(
                `source[type="${pictureSrcType}"]`
              );
